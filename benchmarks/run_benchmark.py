...
def main():
    ...
    if env_workspace:
        ...
        root = Path(
            os.environ.get("ANTIGRAVITY_WORKSPACE_ROOT", project_root / "workspaces")
        )
    ...
    # Provide enough env to satisfy the active runtime contract without
    # depending on legacy paid proxy providers.
    env_overrides = {
...
"""
    for r in results:
        ...
        md_content += (
            f"| {r['name']} | {r['category']} | {status} | {r['latency_seconds']}s |\n"
        )
    ...
    print(f"\n{'=' * 60}")
    print(
        f"  Results: {passed}/{total} passed — {summary['total_latency_seconds']}s total"
    )
